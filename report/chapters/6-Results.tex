...
\begin{figure}
    ...
    \hfill
    \subfloat[Nicely finding a ridge and staying on it.]{
        \includegraphics[width=0.46\linewidth]{fig/EScatchingridge.png}
        \label{fig:EScatchingridge}
    }
    \caption{Various examples of the ES algorithm acting from different starting points.}
...
A good heuristic for whether or not our paths are precise enough would be a measure that would give some confidence interval for final position, given the gist of a trajectory, and the duration of flight along it. We have calculated such measures, presented in \todoref{reflyapunov heuristic}.
